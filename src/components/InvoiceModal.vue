...
export default {
   ...
   closeInvoice() {
    ...
    async uploadInvoice() {
      if (this.invoiceItemList.length <= 0) {
        alert("pls fill out the work items!");
        return;
      }
      this.loading = true;
  ...
  watch: {
    paymentTerms() {
      ...
      this.paymentDueDate = new Date(this.paymentDueDateUnix).toDateString(
        "en_us",
        this.dateOptions
      );
    },
  ...
  created() {
    ...
    this.invoiceDate = new Date(this.invoiceDateUnix).toDateString(
      "en_us",
      this.dateOptions
    );
  },
